...
|속성|기능|기본값|
|----|----|---|
|value|temporalType.DATE  <br>,temporalType.DATE <br>, temporalType.TIMESTAMP |temporalType는 필수로 지정해야 한다.|
...
#### LocalDate , LocalDateTime 사용하기

* JPA 2.1이하의 경우 LocalDate , LocalDateTime 미지원
* Hibernate 5.3 부터 JPA 2.2지원
...
``` java
    ...
    @Entity
    @Table(name = "orders") // expected "identifier"; SQL statement:
    public class Order {
...
하이버네이트가 자동 키 생성 전략을 어떤과정으로 결정하는가?

![A](img/img2.png)


1. @GeneratedValue가 사용된 데이터 타입이 UUID인가?
